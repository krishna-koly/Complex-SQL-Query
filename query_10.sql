...
#Question: Write a query to fetch the number of users who purchased products on multiple days

with cte as(
select *, dense_rank() over(partition by User_id order by Transaction_date) as ok
from transacts
)
